% 
% NOTE: This is not the original - it is an exact copy from the
% mulitband-VERSE repo.
% 
function [C,time,g,slew,k, phi, sta, stb,p_of_t,VERSE_out] = minTimeGradient_VERSE_girf(C,g0, gfin, gmax, smax,T, ds, show,VERSE_in,alpha)
% 
...
%%% SAS add alpha into evaluating W.
alpha_int = interp1(p,alpha,[0:dp:Lp-1]);
W = b./repmat(Gmod(:),[1 size(b,2)]).*alpha_int(:);
...
alpha_int = interp1([0:dp:Lp-1].',alpha_int, p_of_s_half); %<- again avoid spline
...
[phi,k] = sdotMax(PP,p_of_s_half,s_half, gmax, smax,W_of_s_half,bmax,alpha_int);
